<cfcomponent output="false" extends="cfc.ontap">
	...
	<cffunction name="attachIoCContainer" access="public" output="false">
		<cfset var adapter = CreateObject("component","files._tap._cfc.fusebox.appmanager").init() />
		<cfset getIoC().addContainer("fusebox",adapter) />
	</cffunction>
	
	<cffunction name="detachIoCContainer" access="public" output="false">
		<cfset getIoC().detach("fusebox") />
	</cffunction>
	...
	<cffunction name="removeAll" access="public" output="false">
		<cfset var ioc = getIoC().getContainer("fusebox") />
		<cfset var rsFusebox = ioc.getImportedApplicationList() />
		<cfloop query="rsFusebox">
			<cfset ioc.uninstall(rsFusebox.name) />
		</cfloop>
	</cffunction>
</cfcomponent>