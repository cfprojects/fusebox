...
<cf_validate form="#variables.installForm#">
	...
	<cftry>
		...
		<!--- the same form is also used to import / install new fusebox apps, 
		...
		<cfif len(trim(attributes.appname))>
			<cfset getTap().getIoC().getContainer("fusebox").install(attributes.appname,attributes.location,attributes.path) />
		</cfif>
